...
def get_brain(mesh_name: str, anisotropy_type: str, alpha: float):
    ...
    if "debian" in _hostname:
        mesh_directory = Path.home() / "Documents/brain3d/2d-meshes/cylinder"
        # mesh_directory = Path.home() / "Documents/brain3d/2d-meshes/dolfin"
    ...
    # facet_function = df.MeshFunction("size_t", mesh, mesh.geometry().dim() - 1)
    # with df.XDMFFile(str(mesh_directory / f"{mesh_name}_ff.xdmf")) as ff_file:
    #     ff_file.read(facet_function)

    # dirichlet_bc_u_list = [(df.Constant(0), 1), (df.Constant(0), 3)]
    # dirichlet_bc_v_list = [(df.Constant(0), 1), (df.Constant(0), 3)]
    ...
    brain = Model(
        ...
        # dirichlet_bc_u=dirichlet_bc_u_list,
        # dirichlet_bc_v=dirichlet_bc_v_list,
...
def get_solver(*, brain: Model, Ks: float, Ku: float) -> MultiCellSplittingSolver:
    ...
    class PeriodicBoundary(df.SubDomain):
        ...
        def map(self, x, y):
            ...
            # y[0] = x[0] - np.max(brain.mesh.coordinates()[:, 0])     # x-axis
            # y[0] = x[0] - 1.2    # x-axis
            y[0] = x[0] - 1.2
            y[1] = x[1]
    ...
    solver = MultiCellSplittingSolver(
        ...
        periodic_domain=PeriodicBoundary(),
