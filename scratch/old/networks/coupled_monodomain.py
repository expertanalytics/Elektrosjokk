...
class CoupledMonodomainSolver:
    def __init__(
        ...
        interface_tags: InterfaceTags = None,
        ...
        # Check that the conductivity keys match or both are empty
        ...
        # Check cell tags for None
        if cell_tags is None or cell_function is None:
            print("Setting cell tags and cell function to zero.")
            self._cell_tags = CellTags()        # All values should default to zero
            self._cell_function = df.MeshFunction("size_t", mesh, mesh.geometry().dim())
            self._cell_function.set_all(0)      # Set all values to zero
        else:
        ...
        if interface_tags is None or interface_function is None:
            self._interface_tags = InterfaceTags()      # All values should default to zero
            self._interface_function = df.MeshFunction("size_t", mesh, mesh.geometry().dim() - 1)
            self._cell_function.set_all(0)      # Set all values to zero
        else:
