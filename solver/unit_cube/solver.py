...
def setup_application_parameters():
    ...
    application_parameters.add("T", 1e0)                # End time  (ms)
...
def setup_cardiac_model(application_parameters):
    "Return beat model instance"
    # Initialize the computational domain in time and space
    time = Constant(0.0)
    mesh = UnitCubeMesh(10, 10, 10)
    
    inside = CompiledSubDomain("x[0] > 0.25 && x[0] < 0.75 && \
                                x[1] > 0.25 && x[1] < 0.75 && \
                                x[2] > 0.25 && x[2] < 0.75")

    cell_domains = CellFunction("size_t", mesh)
    cell_domains.set_all(2)
    inside.mark(cell_domains, 1)
    ...
    """
    mesh = Mesh()
    hdf = HDF5File(mesh.mpi_comm(), "../convex_hull/beat.h5", "r")
    ...
    # Initialize beat model with the above input
    ...
    return heart, cell_domains, facet_domains
...
def main():
    ...
    brain, cell_domains, facet_domains = setup_cardiac_model(application_parameters)
    ...
    #splitting_solver_params["ode_solver_choice"] = "BasicCardiacODESolver"
    splitting_solver_params["CardiacODESolver"]["scheme"] = "GRL1"   # RushLarsen?
    ...
    splitting_solver_params["apply_stimulus_current_to_pde"] = True
    ...
    brainrestrictor = create_submesh(brain.domain(), cell_domains, 1)
    waterrestrictor = create_submesh(brain.domain(), cell_domains, 2)

    postprocessor.add_fields([
          Restrict("u", brainrestrictor, field_params, name="u_cardiac"),
          Restrict("v", brainrestrictor, field_params, name="v_cardiac"),
